...
fileRouter.get('/:id/download-url', async (req: AuthRequest, res, next) => {
  try {
    // 1. VALIDASI USER (Sangat Penting!)
    // Jangan gunakan '!', gunakan pengecekan manual untuk menghindari crash
    if (!req.user || !req.user.id) {
      console.error("DEBUG: User not found in request. Auth middleware might have failed.");
    ...
    const userId = req.user.id;
    const fileId = String(req.params.id);
    console.log(`DEBUG: Generating download URL for user ${userId}, file ${fileId}`);

    // 2. Cari file dengan aman
    const file = await prisma.file.findFirst({ 
      where: { id: fileId, userId: userId } 
    })
