    ...
    if (!preview || preview.file.status !== "active") {
      return res
        ...
        .json({
          ...
          message: "Preview token not found.",
        });
    }
    return streamProviderFile(preview.file, req.headers.range, res, {
      disposition: "inline",
    });
  } catch (error) {
    return next(error);
  ...
  try {
    ...
    for (const file of files.filter((f) => deletedIds.includes(f.id))) {
      sizeByAccount.set(
        ...
        (sizeByAccount.get(file.connectedAccountId) ?? 0n) + file.sizeBytes,
      );
    }
    ...
    return res.json({
      shares: shares
        ...
        .map((share) => ({
          id: share.id,
          url: share.token
            ? `${env.FRONTEND_URL}/public/files/${share.token}`
            : null,
...
fileRouter.get("/:id/download-url", async (req: AuthRequest, res, next) => {
  try {
    if (!req.user || !req.user.id) {
      return res
        .status(401)
        .json({ code: "AUTH_REQUIRED", message: "Bearer token required." });
    }
...
fileRouter.post("/:id/preview-token", async (req: AuthRequest, res, next) => {
  try {
    ...
    return res
      .status(201)
      .json({ path, url: `${req.protocol}://${req.get("host")}${path}` });
  } catch (error) {
    return next(error);
  }
});
  ...
  try {
    ...
    if (file.provider !== "google_drive") {
      return res
        ...
        .json({
          code: "NO_THUMBNAIL",
          message: "Thumbnails only available for Google Drive files.",
        });
    }
    const auth = await getAuthedGoogleClient(file.connectedAccount);
    const drive = google.drive({ version: "v3", auth });
    ...
    return res
      .status(200)
      .json({ url: thumbnailLink.replace(/=s\d+$/, "=s400") });
  } catch (error) {
    return next(error);
  }
});
...
fileRouter.post("/:id/share", async (req: AuthRequest, res, next) => {
  try {
    ...
    // Set public DULU sebelum cek existing share
    if (file.provider === "google_drive") {
      const auth = await getAuthedGoogleClient(file.connectedAccount);
      const drive = google.drive({ version: "v3", auth });
      console.log("🔓 Setting file public:", file.providerFileId);
...
fileRouter.delete("/:id", async (req: AuthRequest, res, next) => {
  try {
    const fileId = String(req.params.id);
    const file_info = await prisma.file.findFirstOrThrow({
      where: { id: fileId, userId: req.user!.id },
      include: { connectedAccount: true },
    });
